...
import { HomeDashboard } from "@/components/home-dashboard-opportunity" // <--- Will need to pass clientData here
...
import { TaxTransactions }
 from "@/components/tax-transactions"
...
import { InsightsTrendsAnalytics } from "@/components/insights-trends-analytics"
import { InsightsGoalTracking } from "@/components/insights-goal-tracking"

// Import ClientAddModal and Client interface from the new file
import { ClientAddModal, Client } from "@/components/client-add-modal"
...
export default function PlannerCRMApp() {
  ...
  const [currentAccountView, setCurrentAccountView] = useState<"overview" | "details" | "add">("overview")
  const [selectedAccountId, setSelectedAccountId] = useState<string>("")
  const [currentTransactionView, setCurrentTransactionView] = useState<"feed" | "details" | "recurring" | "rules">(
    "feed",
  )
  ...
  const [currentTaxView, setCurrentTaxView] = useState<"overview" | "preparation" | "transactions" | "history">(
    "overview",
  )
  ...
  const [currentDebtView, setCurrentDebtView] = useState<"overview" | "manage" | "loan-details" | "strategy" | "add">(
    "overview",
  )
  const [selectedLoanId, setSelectedLoanId] = useState<string>("")
  const [currentGoalView, setCurrentGoalView] = useState<"overview" | "details" | "ideas" | "history" | "create">(
    "overview",
  )
  ...
  const handleTabChange = (tabId: string) => {
    ...
    if (currentModule === "taxes") {
      setCurrentTaxView(tabId as "overview" | "preparation" | "transactions" | "history")
    }
    if (currentModule === "insurance") {
      setCurrentInsuranceView(tabId as "overview" | "policies" | "needs-assessment")
    }
  ...
  const clientModuleConfig = {
    home: {
      title: "Overview", // This title will be overridden by HomeDashboard's internal tab title
      tabs: [], // HomeDashboard now manages its own tabs
      rightAction: null,
    },
    insights: {
      title: "Insights",
      tabs: [
        { id: "report", label: "Monthly Report", isActive: currentTab === "report" },
        { id: "trends", label: "Trends & Analytics", isActive: currentTab === "trends" },
        { id: "goals", label: "Goal Tracking", isActive: currentTab === "goals" },
      ],
    ...
    investments: {
      title: "Investments",
      tabs: [
        { id: "overview", label: "Overview", isActive: currentTab === "overview" },
        { id: "all-holdings", label: "All Holdings", isActive: currentTab === "all-holdings" },
      ],
    ...
    transactions: {
      title: "Transactions",
      tabs: [
        { id: "feed", label: "Transaction Feed", isActive: currentTab === "feed" },
        { id: "recurring", label: "Recurring", isActive: currentTab === "recurring" },
        { id: "rules", label: "Categorization Rules", isActive: currentTab === "rules" },
      ],
    ...
    spending: {
      ...
      tabs: [
        { id: "overview", label: "Overview", isActive: currentTab === "overview" },
        { id: "breakdown", label: "Breakdown & Trends", isActive: currentTab === "breakdown" },
        { id: "merchants", label: "Merchants", isActive: currentTab === "merchants" },
      ],
    ...
    debts: {
      ...
      tabs: [
        { id: "overview", label: "Overview", isActive: currentTab === "overview" },
        { id: "manage", label: "Manage Debts", isActive: currentTab === "manage" },
        { id: "strategy", label: "Strategy & Acceleration", isActive: currentTab === "strategy" },
      ],
    ...
    budgets: {
      ...
      tabs: [
        { id: "overview", label: "Overview", isActive: currentTab === "overview" },
        { id: "manage", label: "Manage Budget", isActive: currentTab === "manage" },
        { id: "insights", label: "Insights & Projections", isActive: currentTab === "insights" },
      ],
    ...
    retirement: {
      ...
      tabs: [
        { id: "overview", label: "Overview", isActive: currentTab === "overview" },
        { id: "plan-details", label: "Plan Details", isActive: currentTab === "plan-details" },
        { id: "lifestyle", label: "Lifestyle Visualizer", isActive: currentTab === "lifestyle" },
      ],
    ...
    taxes: {
      ...
      tabs: [
        { id: "overview", label: "Overview", isActive: currentTab === "overview" },
        { id: "preparation", label: "Tax Preparation", isActive: currentTab === "preparation" },
        { id: "transactions", label: "Tax Transactions", isActive: currentTab === "transactions" },
        { id: "history", label: "History & Documents", isActive: currentTab === "history" },
      ],
    ...
    insurance: {
      ...
      tabs: [
        { id: "overview", label: "Overview", isActive: currentTab === "overview" },
        { id: "policies", label: "My Policies", isActive: currentTab === "policies" },
        { id: "needs-assessment", label: "Needs Assessment", isActive: currentTab === "needs-assessment" },
      ],
  ...
  const renderCRMContent = () => {
    switch (currentCRMModule) {
      ...
      case "clients":
        ...
        return <ClientsList clients={clients} onNavigateToClient={handleClientSelection} onAddClient={openAddClientModal} />
      case "prospects":
  ...
  const renderClientContent = () => {
    if (currentModule === "home") {
      return <HomeDashboard clientData={currentClient || undefined} /> {/* <-- Pass clientData here */}
    }
    ...
    if (currentModule === "insights") {
      ...
      if (currentTab === "trends") {
        return <InsightsTrendsAnalytics />
      }
      if (currentTab === "goals") {
        return <InsightsGoalTracking />
      }
      return (
        <div className="flex items-center justify-center h-96 text-muted-foreground">
          <div className="text-center">
            <h3 className="text-lg font-medium mb-2">
              {clientModuleConfig[currentModule as keyof typeof clientModuleConfig]?.title} -{" "}
              {
                clientModuleConfig[currentModule as keyof typeof clientModuleConfig]?.tabs.find(
                  (t) => t.id === currentTab,
                )?.label
              }
            </h3>
    ...
    if (currentModule === "goals") {
      if (currentGoalView === "details") {
        return <GoalDetails goalId={selectedGoalId} onBack={() => setCurrentGoalView("overview")} />
      }
        ...
        return (
          <div className="flex items-center justify-center h-96 text-muted-foreground">
            <div className="text-center">
              ...
              <Button onClick={() => setCurrentGoalView("overview")} className="mt-4">
                Back to Overview
              </Button>
            </div>
          </div>
    ...
    if (currentModule === "accounts") {
      if (currentAccountView === "details") {
        return <AccountDetails accountId={selectedAccountId} onBack={() => setCurrentAccountView("overview")} />
      }
    ...
    if (currentModule === "transactions") {
      if (currentTransactionView === "details") {
        return (
          <TransactionDetails transactionId={selectedTransactionId} onBack={() => setCurrentTransactionView("feed")} />
        )
      }
      if (currentTransactionView === "recurring") {
        return <RecurringTransactions onBack={() => setCurrentTransactionView("feed")} />
      }
      if (currentTransactionView === "rules") {
        return <CategorizationRules onBack={() => setCurrentTransactionView("feed")} />
      }
      return (
        <TransactionsFeed
    ...
    if (currentModule === "debts") {
      ...
      if (currentDebtView === "loan-details") {
        return <LoanDetails loanId={selectedLoanId} onBack={() => setCurrentDebtView("manage")} />
      }
        ...
        return (
          <div className="flex items-center justify-center h-96 text-muted-foreground">
            <div className="text-center">
              ...
              <Button onClick={() => setCurrentDebtView("overview")} className="mt-4">
                Back to Overview
              </Button>
            </div>
          </div>
    ...
    if (currentModule === "retirement") {
      if (currentTab === "plan-details") {
        return <RetirementPlanDetails onBack={() => setCurrentTab("overview")} />
      }
      if (currentTab === "lifestyle") {
        return <RetirementLifestyleVisualizer onBack={() => setCurrentTab("overview")} />
      }
      return (
        <RetirementOverview
    ...
    if (currentModule === "insurance") {
      ...
      if (currentInsuranceView === "policy-details") {
        return <PolicyDetails policyId={selectedPolicyId} onBack={() => setCurrentInsuranceView("policies")} />
      }
      if (currentInsuranceView === "needs-assessment") {
        return <InsuranceNeedsAssessment onBack={() => setCurrentInsuranceView("overview")} />
      }
      return (
        <InsuranceOverview
          onNavigateToPolicies={() => setCurrentInsuranceView("policies")}
          onNavigateToNeedsAssessment={() => setCurrentInsuranceView("needs-assessment")}
          onNavigateToAddPolicy={() => {
            console.log("Add policy")
          }}
          onNavigateToPolicyDetails={(policyId) => {
  ...
  const currentConfig =
    currentView === "crm"
      ? crmModuleConfig[currentCRMModule as keyof typeof crmModuleConfig] || crmModuleConfig.dashboard
      : clientModuleConfig[currentModule as keyof typeof clientModuleConfig] || clientModuleConfig.home
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="pt-16">
        <SidebarProvider>
          {currentView === "crm" ? (
            <PlannerSidebar onNavigate={handleCRMNavigation} currentModule={currentCRMModule} />
          ) : (
            <AppSidebar onNavigate={handleClientNavigation} currentModule={currentModule} />
          )}
          ...
          <SidebarInset>
            <header className="flex shrink-0 gap-2 border-b px-4 h-12 items-center">
              <div className="flex items-center justify-between flex-1">
                <div className="flex items-center space-x-6">
                  ...
                  {currentModule === "home" ? (
                      <h1 className="text-lg font-semibold">{currentClient?.name ? `${currentClient.name}'s Dashboard` : "Client Dashboard"}</h1>
                  ) : (
                  ...
                  {currentModule !== "home" && currentConfig.tabs.length > 0 && (
                    <ModuleTabs tabs={currentConfig.tabs} onTabChange={handleTabChange} />
                  )}
                </div>

                <div className="flex items-center space-x-4">
                  {currentConfig.rightAction}
                  <div className="text-sm font-medium text-gray-600">{getCurrentDate()}</div>
                </div>
              </div>
            </header>
            ...
            {isTransitioning ? (
              ...
              <main className="flex-1 p-6">{currentView === "crm" ? renderCRMContent() : renderClientContent()}</main>
            )}
          </SidebarInset>
        </SidebarProvider>
      </div>
